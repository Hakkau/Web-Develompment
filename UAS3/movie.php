...
<?php
	...
	if(isset($_POST['Simpan']))
	{
		...
		if(($ekstensifile == "jpg") or ($ekstensifile == "JPG") or ($ekstensifile == "png") or ($ekstensifile == "PNG") or ($ekstensifile == "jpeg") or ($ekstensifile == "JPEG") or ($ekstensifile == "jfif"))
		{
			...
			mysqli_query($connection, "insert into movie value ('$movieKODE', '$movieNAMA', '$movieFILE')");
...
// ----------------- AKHIR PAGING -- MEMBUAT GANTI HALAMAN ---------------- >
			...
			if (isset($_POST['kirim'])) {
				...
				where movieKODE like '%".$search."%' limit $mulaitampilan, $jumlahtampilan");
			} elseif (isset($_POST['kirim2'])) {
				$search2 = $_POST["search2"];
				$query = mysqli_query($connection,"select * from movie
				where movieNAMA like '%".$search2."%' limit $mulaitampilan, $jumlahtampilan");
			} else {
				$query = mysqli_query($connection, "select * from movie limit $mulaitampilan, $jumlahtampilan");
			}
			$nomor = 1;
